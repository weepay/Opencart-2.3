...
class ControllerExtensionPaymentWeepayPayment extends Controller
{
    ...
    public function index()
    {
        ...
        if (VERSION >= '2.2.0.0') {
            ...
            $template_url = 'default/template/extension/payment/weepay_payment.tpl';
        }
    ...
    public function checkoutform()
    {
        try {
            ...
            $this->load->language('extension/payment/weepay_payment');
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/weepay_payment.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/extension/payment/weepay_payment.tpl';
            } else {
                $this->template = 'default/template/extension/payment/weepay_payment.tpl';
            }
            $this->load->model('checkout/order');
            $this->load->model('checkout/order');
